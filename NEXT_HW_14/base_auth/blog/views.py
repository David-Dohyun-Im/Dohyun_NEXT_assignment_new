...
from .utils import update_post_lasted_viewed,get_new_file_name
...
def detail(request, post_pk):
    ...
    if request.method == "POST":
        ...
        image_url = None
        if image:
            image_name = image_name = get_new_file_name(image, request.user, content)
            saved_path = default_storage.save(image_name, image)
            image_url = default_storage.url(saved_path)
        Comment.objects.create(post=post, content=content, creator=request.user, image_url=image_url)
